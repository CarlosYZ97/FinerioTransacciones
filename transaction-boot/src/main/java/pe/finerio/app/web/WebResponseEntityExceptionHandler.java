...
import pe.finerio.app.web.core.WebException;
import pe.finerio.app.web.exception.CustomizableHttpResponseException;
import pe.finerio.app.web.message.WebMessageSource;
import pe.finerio.app.web.model.WebResponseEntity;
...
public abstract class WebResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers, HttpStatus status, WebRequest request) {
        ...
        if (ex instanceof CustomizableHttpResponseException) {
            ...
            webResponseEntity.setMessage(message.getMessage());
            if (shortMessage != null && shortMessage.equals(ERROR_INESPERADO)) {
                webResponseEntity.setShortMessage(shortMessage);
            } else {
                webResponseEntity.setShortMessage(message.getShortMessage());
            }

            webResponseEntity.setErrorCode(errorCode);
            webResponseEntity.setTrackingId(this.getTrackingId());
            log.error("code: [{}] message: [{}]", new Object[]{webResponseEntity.getErrorCode(), webResponseEntity.getMessage(), ex});
